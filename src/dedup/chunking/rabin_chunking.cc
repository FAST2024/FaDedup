...
static int chunk_avg_size = 0;
static int chunk_min_size = 0;
...
void rabin_init(Config* conf) {
	...
	windows_reset();
	_num_chunks = 0;
    chunk_avg_size = conf->_chunkAvgSize;
    chunk_max_size = conf->_chunkMaxSize;
    chunk_min_size = conf->_chunkMinSize;
	rabin_mask = chunk_avg_size - 1;
}
...
int rabin_chunk_data(char *p, int n) {
	...
	int end = n > chunk_max_size ? chunk_max_size : n;
	while (i < end) {
