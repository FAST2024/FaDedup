...
int RedisUtil::_redis_context_cnt=0;
std::mutex RedisUtil::_redis_cnt_mutex;
...
redisContext* RedisUtil::createContext(string ip, int port) {
  // cout << "[RedisUtil] try to create context to " << ip << endl;
  redisContext* retVal = redisConnect(ip.c_str(), port);
  if (retVal == NULL) {
      cerr << "[RedisUtil] create context to " << ip << " error, return null" << endl;
      exit(-1);
  }
  if(retVal -> err) {
    cerr << "[RedisUtil] create context to " << ip << " error, Error: " << retVal -> errstr << endl;
    exit(-1);
  }
  return retVal;
}
